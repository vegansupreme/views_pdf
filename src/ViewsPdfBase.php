...
//     );
    ...
    {
        if (Unicode::strlen($hex) == 6) {
            $r = Unicode::substr($hex, 0, 2);
            $g = Unicode::substr($hex, 2, 2);
            $b = Unicode::substr($hex, 4, 2);

            return array(hexdec($r), hexdec($g), hexdec($b));

        } elseif (Unicode::strlen($hex) == 3) {
            $r = Unicode::substr($hex, 0, 1);
            $g = Unicode::substr($hex, 1, 1);
            $b = Unicode::substr($hex, 2, 1);
        ...
        foreach ($rows as $row) {
            ...
            foreach ($columns as $id => $column) {
                ...
                if (!empty($column->options['exclude']) && is_object($view->field[$id])) {
                    // Render the element, but don't print the output. This
